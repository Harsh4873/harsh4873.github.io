...
function syncModelFocusStatus(model) {
...
function setModelFocusButtonState(model, isRunning, label) {
  const card = getSelectedModelCard();
...
function refreshModelFocusFromSelected() {
  const card = getSelectedModelCard();
...
async function runSelectedModel(event) {
  const card = getSelectedModelCard();
  if (!card) return;
  const key = String(card.dataset.modelKey || '').trim();
  ...
  if (action === 'cannon') {
    ...
    try {
      await loadCannonDailyPicks();
    } finally {
      refreshModelFocusFromSelected();
    }
    return;
  }
  if (key) await runModel(key, event || {});
}
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initModelWheel);
} else {
  initModelWheel();
}
