...
- Do not add or modify Daymark, Fare, Sift, Gym, or Portfolio source, data, or styling from this branch — with one exception: `firestore.rules` here intentionally carries the complete Daymark + Slate + Fare + Sift ruleset and must stay identical to the copies on the `daymark`, `fare`, and `research` branches.
