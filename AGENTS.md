...
- Do not add or modify Daymark, Fare, Gym, or Portfolio source, data, or styling from this branch — with one exception: `firestore.rules` here intentionally carries the complete Daymark + Slate + Fare ruleset and must stay identical to the copies on the `daymark` and `fare` branches.
